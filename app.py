...
def update_martingale_balance(actual):
    """Custom martingale: 0.01 → 0.02 → stop until under_avg >= 8"""
...
# --- Streamlit App ---
def main():
    st.title("Crash Predictor — Under Count Strategy with Martingale 0.01→0.02→stop")
    ...
    st.caption("Martingale bets sequence: 0.01 → 0.02 → stop until under average ≥ 8. Bets only when 'Above' is predicted and result recorded.")
